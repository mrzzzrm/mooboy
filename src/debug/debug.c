...
//    snap_mem(dbg.before.mem);
    dbg.before.cpu = cpu;
    dbg.op = mem_read_byte(dbg.before.cpu.pc.w);
//    sym_before();
//    int_before();
//    mon_before();
//    break_before();
...
void debug_after() {
    ...
    printf("[%.4X %.4X %.4X %.4X] RB: %i\n", mem_read_word(sp), mem_read_word(sp + 2), mem_read_word(sp + 4), mem_read_word(sp + 6), (u8(*)[0x4000])mbc.rombank - card.rombanks);
